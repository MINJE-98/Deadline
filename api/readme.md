...
| --- | --- | --- | --- |
| 생성 | 유저(필수) : 유저는 여러 팀을 생성/참여할 수있다.
팀(선택) : 팀은 여러 유저를 가질 수 있다. | N:M |  |
| 생성 | 유저(필수) : 유저는 여러 팀에 생성, 참여할 수 있다.
팀멤버(선택) : 팀멤버는 한 유저만 가질 수 있다. | 1:N |  |
| 참조 | 팀(필수) : 한팀은 여러 유저를 갈 수 있다.
팀멤버(선택) : 팀멤버 |  |  |
| 추가/변경 | 팀(필수) : 팀은 여러개의 아이템을 추가할 수 있다.
아이템(선택) : 아이템은 한번 추가하면 추가할수없다. | 1:N |  |
| 추가/변경 | 팀(필수) : 팀은 여러개의 태그를 추가 할 수 있다.
태그(선택) : 태그는 하나의 팀에만 존재해야함. | 1:N | 태그는 팀없이 존재할 수 없다. |
| 추가/변경 | 아이템이미지(필수) : 아이템이미지는 여러개의 유통기한을 가질 수 있다.
